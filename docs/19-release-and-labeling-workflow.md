...
### ステップ 4: バージョンバンプPRのマージと自動リリース
1. 通常の機能・修正PRが `main` ブランチへマージされると、**Release Workflow** が起動し、バージョンおよび `CHANGELOG.md` を更新する「`bump: version to vX.Y.Z`」PRが自動作成されます。
2. 作成されたバージョンバンプPRの内容（バージョン番号と変更履歴）を確認し、マージします。
3. バージョンバンプPRが `main` へマージされると、ワークフローが自動的に注釈付き Git タグ (`vX.Y.Z`) を作成してリモートに Push し、そのタグトリガーによって Gem のビルド・パッケージ公開および GitHub Release の正式公開（Latest リリース化）が完了します。
