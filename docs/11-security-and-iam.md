...
ステージ（`production`, `staging` 等）に応じて厳格なセキュリティ要件を強制するための `StagePolicy` を定義できます。バリデーションフェーズにおいてポリシー違反が検出された場合、エラー Diagnostic（`VLT-IAM-001`, `VLT-SCHED-002`, `VLT-BUILD-001` 等）が出力され、CLI は終了コード `8`（`EXIT_POLICY_VIOLATION`）で停止します。
