...
通常は人間に読みやすいテキスト形式（標準出力）で出力されます。`--format json` オプションを指定すると、CI/CDツールとの連携に適した構造化JSON形式で出力されます。

### JSON出力の共通スキーマ

全コマンドの JSON 出力は以下の共通スキーマに基づき、`jq` などのツールで直接パース可能です。
...
- **`command`**: 実行された CLI コマンド名（例: `"deploy"`, `"build"`, `"validate"` 等）
- **`status`**: コマンドの実行結果（`"success"` または `"error"`）
- **`diagnostics`**: 診断情報オブジェクト（`Diagnostic`）の配列。エラーや警告が含まれます（存在しない場合は空配列 `[]`）
- **`data`**: コマンド固有の構造化データ（成功時のアーティファクト情報や実行結果、失敗時のエラーコードおよびメッセージ等）

### 機密情報の自動マスク

`Veltrunodefile` の `env(..., secret: true)` で secret マークされた値や機密情報は、JSON 出力時（`data`、`diagnostics`、メッセージ文字列等）に自動的に `[FILTERED]` としてマスクされ、CI/CD のログや標準出力にシークレットが平文で露出することを防止します。
