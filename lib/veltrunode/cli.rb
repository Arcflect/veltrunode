  ...
  class CLI
    ...
    class Router
      ...
      def parse_global_options!
        ...
        # バージョンフラグの抽出
        if @argv.include?("--version") || @argv.include?("-v") || @argv.include?("version")
      ...
      def execute_efs_verify
        name = @argv.first || "default"
        output_success("EFS verification successful for: #{name}.", { message: "EFS verification successful for: #{name}" })
      end
